...
if [ ! -d "./result/${server_country}_${resolver}/" ]; then
    ...
    mkdir ./result/${server_country}_${resolver}
fi
...
do
    ...
    sudo ./ztls_tfo_client $domain_name $port > ./result/${server_country}_${resolver}/ztls_tfo_$formatted_test_num.txt;
    sleep 1
    sudo ./ztls_tcp_client $domain_name $port > ./result/${server_country}_${resolver}/ztls_tcp_$formatted_test_num.txt;
    sleep 1
    sudo ./tls_tcp_client $domain_name $port > ./result/${server_country}_${resolver}/tls_tcp_$formatted_test_num.txt;
    sleep 1
    c=$(expr $c + 1)
